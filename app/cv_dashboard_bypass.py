...
def drive_loop():
    ...
    while True:
        ...
        if (cfg["running"] and
            ...
            config["running"] = False   # 주행 중단
            motor.motorStop()
            time.sleep(0.5)             # 완전 정지 대기

            def run_ai():
                try:
                    time.sleep(1.0)   # Gemini 쿨다운
                    from ai_core.engine import AGVAIEngine

                    engine = AGVAIEngine(mode=config.get("user_mode", "SAFE").upper())

                    frame_snap = cam.capture()
                    frame_snap = cv2.cvtColor(frame_snap, cv2.COLOR_RGB2BGR)
                    ...
                    result = engine.evaluate_state_and_calculate_output(
                        sensor_snapshot, frame_snap
                    )

                    print(f"[AI] state={result.get('state')} route={result.get('route')} speed={result.get('speed_limit_pct')}")
                    with lock:
                        state["ai_state"] = result.get("state", "--")
                        state["ai_route"] = result.get("route", "--")
                        state["ai_speed"] = str(result.get("speed_limit_pct", "--"))

                    if result.get("state") in ("PATH_BLOCKED", "REROUTING_RUN"):
                        ...
                        new_route_str = reroute_map.get((cur_route, route_key))
                        if new_route_str:
                            threading.Thread(
                            ...
                            config["running"] = True
                    elif result.get("state") == "CRITICAL_STOP":
                        motor.motorStop()
                        config["running"] = False
                    else:
                        spd = result.get("speed_limit_pct", config["base_speed"])
                        if spd:
                            config["base_speed"] = int(spd)
                        config["running"] = True
            ...
            threading.Thread(target=run_ai, daemon=True, name="ai").start()
...
<div class="layout">
  ...
  <div class="card">
    ...
    <div class="status-grid">
      ...
      <div class="stat" style="grid-column: 1 / -1;">
        <div class="label">AI 판단</div>
        <div class="value" id="ai-state" style="font-size:0.85rem;">--</div>
      </div>
    </div>
  ...
  <div class="card controls">
    ...
    <div style="font-size:0.7rem;color:#666;margin-bottom:6px">주행 모드</div>
    <div class="btn-row" style="margin-bottom:12px">
      <button id="btn-safe" onclick="setMode('safe')"
        style="flex:1;padding:12px;border:2px solid #0af;background:#0f2030;
               color:#0af;border-radius:6px;cursor:pointer;font-family:inherit;
               font-size:0.85rem;font-weight:bold;">
        🛡 SAFE (안전)
      </button>
      <button id="btn-fast" onclick="setMode('fast')"
        style="flex:1;padding:12px;border:2px solid #2a2a3a;background:#0f0f13;
               color:#666;border-radius:6px;cursor:pointer;font-family:inherit;
               font-size:0.85rem;font-weight:bold;">
        ⚡ FAST (빠르게)
      </button>
    </div>
...
function updateState() {
  fetch('/state').then(r=>r.json()).then(d=>{
    ...
    ultraEl.className = 'value' + (dist >= 0 && dist < 30 ? ' warn' : dist >= 0 && dist < 15 ? ' danger' : '');

    const aiEl = document.getElementById('ai-state');
    if (d.ai_state && d.ai_state !== '--') {
      aiEl.textContent = `${d.ai_state} | 경로: ${d.ai_route} | 속도: ${d.ai_speed}%`;
      aiEl.style.color = d.ai_state.includes('BLOCK') || d.ai_state.includes('REROUTE') ? '#f87171'
                       : d.ai_state.includes('CAUTIOUS') ? '#f59e0b' : '#4ade80';
    }
    }).catch(()=>{});
}
...
function setMode(mode) {
  fetch('/config', {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({user_mode: mode})
  });
  document.getElementById('btn-safe').style.borderColor = mode==='safe' ? '#0af' : '#2a2a3a';
  document.getElementById('btn-safe').style.color       = mode==='safe' ? '#0af' : '#666';
  document.getElementById('btn-safe').style.background  = mode==='safe' ? '#0f2030' : '#0f0f13';
  document.getElementById('btn-fast').style.borderColor = mode==='fast' ? '#f59e0b' : '#2a2a3a';
  document.getElementById('btn-fast').style.color       = mode==='fast' ? '#f59e0b' : '#666';
  document.getElementById('btn-fast').style.background  = mode==='fast' ? '#1a1200' : '#0f0f13';
}
</script>
</body>
</html>'''
