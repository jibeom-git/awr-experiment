...
# 자율 주행 대시보드
#
# 실행: python app/auto_dashboard.py
# 접속: http://192.168.0.50:5003
#
# 사용자는 fast/safe 모드만 선택
# 로봇이 A → B → C 순으로 자율 판단하며 주행
...
# ── USB 웹캠 ───────────────────────────────────────────
try:
    webcam = cv2.VideoCapture("/dev/video2", cv2.CAP_V4L2)
...
def webcam_loop():
    ...
    _wcam = cv2.VideoCapture("/dev/video2", cv2.CAP_V4L2)
...
if __name__ == '__main__':
    print("자율주행 대시보드 시작: http://192.168.0.50:5003")
