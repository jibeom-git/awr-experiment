# app/auto_dashboard_v6.py
# 자율 주행 대시보드 - 양방향 카메라 동시 송출 버그 완치 및 캘리브레이션 통합본
#
# 실행: python app/auto_dashboard_v6.py
...
ROUTES = {
    ...
    "C→start": {1: "right", 2: "straight", 3: "stop"},
}
                ...
                if len(samples) >= 5:
                    ...
                    if variation < 0.05:
                        ...
                        with lock:
                            ...
                            state["admin_waiting"] = True  # 입력 대기 플래그
...
def admin_command():
    ...
    def _parse():
        try:
            ...
            cmd = CommanderVLM()
            # 멈춰/돌아와 판단용 프롬프트
            result = cmd.parse_admin(text)
            action = result.get('action', 'stop')
            with lock:
                state["admin_waiting"] = False
            if action == 'stop':
                with lock:
                    state["ai_status"] = "관리자 명령: 정지"
                    state["action"] = "최종정지"
                config["running"] = False
            elif action == 'return':
                with lock:
                    state["ai_status"] = "관리자 명령: 우회"
                config['route'] = 'C→stop'
                threading.Thread(target=do_reroute, args=('C→stop',),
                                 daemon=True, name="admin_reroute").start()
        except Exception as e:
            print(f"[Admin] 오류: {e}")
            with lock:
                state["admin_waiting"] = False
    ...
    <div class="card">
      ...
      <div class="ai-section">
        ...
        <div id="admin-panel" style="display:none;margin-top:8px;">
          <div style="font-size:.7rem;color:var(--red);margin-bottom:6px;">⚠️ 관리자 명령을 입력하세요</div>
          <div style="display:flex;gap:8px;">
            <input type="text" id="admin-input"
              placeholder="예: 멈춰 / 돌아와"
              style="flex:1;background:rgba(255,61,87,.1);border:1px solid var(--red);
                     border-radius:6px;padding:8px 12px;color:var(--text);
                     font-family:inherit;font-size:.8rem;outline:none;">
            <button onclick="sendAdminCommand()"
              style="padding:8px 16px;border:none;border-radius:6px;
