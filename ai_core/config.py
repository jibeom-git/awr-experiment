...
SPEED_SAFE_LOW       = 30   # 평지 비닐 노면 임계 저속 서행
SPEED_HEAVY_HILL     = 40   # 고중량 경사 감속 상한 (SPEED_HILL_HEAVY_CAP 별칭)
SPEED_HILL_HEAVY_CAP = 30   # 하위 호환 유지
SPEED_DEFAULT        = 50   # SAFE 평지 기본 크루징
SPEED_MEDIUM_POWER   = 45   # B 경로 10도 언덕 정속
SPEED_HIGH_POWER     = 60   # FAST 평지 / A 경로 고경사 돌파
OBSTACLE_BACKUP_SPEED = 40  # 장애물 확인 후진 속도
OBSTACLE_BACKUP_SEC   = 4.0 # 장애물 확인 후진 시간(초)
...
TH_SONIC_CRITICAL    = 0.0          # 충돌 방지 비상 제동 거리 (cm)
TH_SONIC_SLOWDOWN    = 20.0          # VLM 호출 구간 시작 거리 (cm)
...
# ── cv_dashboard 통신 URL (포트 5001) ────────────────────────────────────────
CV_DASHBOARD_URL = "http://localhost:5001"
