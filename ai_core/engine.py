...
from . import config
#from .client import GeminiClient  # google 모듈 충돌로 비활성화            # 기존 app/dashboard.py 하위호환용
...
class AGVAIEngine:
    ...
    def __init__(self, mode: str = "SAFE", sensors: dict | None = None):
        ...
        #self.gemini = None  # google 모듈 충돌로 비활성화          # 하위호환용 구형 Gemini 클라이언트
    ...
    # =========================================================================
    # ⓪ cv_dashboard HTTP 통신 메서드 (포트 5001)
    # =========================================================================

    def _cv_post(self, endpoint: str, data: dict | None = None) -> bool:
        """cv_dashboard에 POST 요청 전송. 실패 시 조용히 폴백"""
        import requests
        try:
            url = config.CV_DASHBOARD_URL + endpoint
            if data:
            ...
            print(f"[Engine] cv_dashboard 통신 실패 {endpoint}: {e}")
            return False

    def _cv_get_state(self) -> dict:
        """cv_dashboard 현재 상태 조회"""
        import requests
        try:
            r = requests.get(config.CV_DASHBOARD_URL + "/state", timeout=2)
            return r.json()
        except Exception:
            return {}

    def send_route_and_speed(self, route_str: str, speed: int, reroute: bool = False):
        """
        경로/속도를 cv_dashboard에 전송.
        reroute=True : 장애물 차단으로 인한 우회 → /reroute {"route": "A→B"} 호출
                       cv_dashboard 가 stop/reset/restart 를 내부 처리
        reroute=False: 속도 변경만 → /config {"base_speed": N} 호출

        우회 경로 매핑 (route_availability 로 차단 경로 자동 탐지):
          ROUTE_A 차단 → /reroute {"route": "A→B"}
          ROUTE_B 차단 → /reroute {"route": "B→C"}
          ROUTE_C 차단 → DEADLOCK (우회 없음, /config 로 폴백)
        """
        route_map = {
            config.ROUTE_A: "A",
            config.ROUTE_B: "B",
        ...
        cv_route_new = route_map.get(route_str, "C")

        if reroute:
            # route_availability 에서 차단된 경로 키 탐색
            blocked_keys = [k for k, v in self.route_availability.items() if not v]
            reroute_str = None
            for blk in blocked_keys:
                candidate = f"{blk}→{cv_route_new}"
                # cv_dashboard 가 지원하는 우회 경로 ("A→B", "A→C", "B→C") 만 허용
                if candidate in ("A→B", "A→C", "B→C"):
                    reroute_str = candidate
                    break
            if reroute_str:
                self._cv_post("/reroute", {"route": reroute_str})
                print(f"[Engine] cv_dashboard 우회 경로 전달: /reroute route={reroute_str}")
            else:
                # 지원하지 않는 우회 조합 → 속도만 전달
                self._cv_post("/config", {"base_speed": speed})
                print(f"[Engine] 우회 경로 없음, /config speed={speed} (route={route_str})")
        else:
            # 속도만 변경
            self._cv_post("/config", {"base_speed": speed})
            print(f"[Engine] cv_dashboard 속도 전달: /config speed={speed}")

    def start_tracking(self):
        """cv_dashboard 라인트래킹 주행 시작"""
        self._cv_post("/start")
        print("[Engine] cv_dashboard 주행 시작")

    def stop_tracking(self):
        """cv_dashboard 라인트래킹 주행 정지"""
        self._cv_post("/stop")
        print("[Engine] cv_dashboard 주행 정지")

    def reset_tracking(self):
        """cv_dashboard 분기점 카운트 리셋 (경로 우회 시 필수)"""
        self._cv_post("/reset")
        print("[Engine] cv_dashboard 분기점 리셋")

    def backup_for_obstacle(self):
        """장애물 확인용 후진 준비: cv_dashboard 정지 후 0.2초 대기"""
        self._cv_post("/stop")
        time.sleep(0.2)
        print("[Engine] cv_dashboard 정지 — 후진 대기 중")
    ...
    def _apply_safety_override(
        ...
        if in_obs_zone:
            ...
            if not passable:
                # XGBoost 예측이 아닌 현재 활성 경로(실제 주행 중인 경로)를 차단
                # → 매번 같은 경로만 차단되는 버그 방지
                blocking_route = (
                    self.active_route
                    if self.active_route not in ("UNKNOWN",)
                    else route
                )
                route_key = blocking_route.replace("ROUTE_", "")
    ...
    def _handle_mock_obs(self, mock_obs, pitch, weight, mode):
        ...
        # 2cm 방지턱: AGV 지상고(3cm) 대비 리스크 높음 → 통과 불가
        if mock_obs == "Bump_2cm":
            return False, "bump_2cm", "2cm 방지턱 — 진동 충격 위험으로 통과 불가", config.SPEED_STOP
        # 1cm 방지턱: 통과 가능, SAFE+고중량이면 최소 속도로 감속
        if mock_obs == "Bump_1cm":
            spd = (config.SPEED_HEAVY_HILL
                   if (mode == config.MODE_SAFE and weight > config.TH_LOAD_HEAVY)
                   else config.SPEED_DEFAULT)
            return True, "bump_1cm", "1cm 방지턱 — 서행 통과 가능", spd
        if mock_obs == "Vinyl":
            if abs(pitch) >= config.TH_PITCH_HILL:
                return False, "vinyl_on_hill", "경사 비닐 노면 — 슬립 위험으로 통과 불가", config.SPEED_STOP
            return True, "vinyl_flat", "평지 비닐 노면 — 초저속 통과", config.SPEED_SAFE_LOW
    ...
    def evaluate_state_and_calculate_output(
        ...
        if in_obs_zone or mock_obs != "None":
            if mock_obs == "Bump_5cm":
                ...
                try:
                    import cv2 as _cv2
                    _, jpeg = _cv2.imencode('.jpg', ai_camera_frame,
                                           [_cv2.IMWRITE_JPEG_QUALITY, 85])
                    vlm = self.vlm.analyze_obstacle(jpeg.tobytes(), ctx, self.mode)
                    self.last_vlm_result = vlm
                except Exception as _e:
                    print(f"[Engine] VLM 오류: {_e}")
                    vlm = {"obstacle_type": "unknown", "passable": False,
                           "recommended_speed_limit": 15, "reason": str(_e),
                           "height_cm": 0.0, "confidence": 0.0}
                                    
            obs_type  = vlm.get('obstacle_type', 'unknown')
            height_cm = float(vlm.get('height_cm', 0.0))
            vlm_speed = vlm.get('recommended_speed_limit', config.SPEED_SAFE_LOW)
            reason    = vlm.get('reason', 'VLM_DECISION')

            # XGBoost로 최종 통과 여부 판단
            obs_enc  = OBS_ENC.get(obs_type, 0)
            mode_enc = MODE_ENC.get(self.mode, 0)
            features = [pitch, weight, ultrasonic, obs_enc, mode_enc,
                        1, float(vlm.get('confidence', 0.5)), 0.0]
            xgb_route, xgb_speed = self.trainer.predict_route_speed(features)

            # XGBoost가 현재 경로 유지하면 통과, 다른 경로면 우회
            if xgb_route and xgb_route != self.active_route:
                passable = False
                vlm_speed = xgb_speed if xgb_speed else vlm_speed
            else:
                passable = True
                vlm_speed = xgb_speed if xgb_speed else vlm_speed
